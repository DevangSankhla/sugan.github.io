...
export default function RatingBreakdown({ rating, reviews }: RatingBreakdownProps) {
  ...
  return (
    <div className="bg-white rounded-xl p-6 border border-sugan-ink/10">
      <h3 className="font-display text-lg text-sugan-ink mb-4">Customer Reviews</h3>

      {/* Overall Rating */}
      <div className="flex items-center gap-4 mb-6">
        <div className="text-center">
          <span className="font-display text-5xl text-sugan-ink">{rating.toFixed(1)}</span>
          ...
          <p className="text-xs text-sugan-ink/50 font-body mt-1">{reviews} reviews</p>
        </div>
        ...
        <div className="flex-1 space-y-1">
          {breakdown.map((item) => (
            <div key={item.stars} className="flex items-center gap-2">
              <span className="text-xs font-body text-sugan-ink/60 w-3">{item.stars}</span>
              <Star className="w-3 h-3 text-sugan-gold fill-sugan-gold" />
              <div className="flex-1 h-2 bg-sugan-bone rounded-full overflow-hidden">
                <div
                  className="h-full bg-sugan-gold rounded-full"
                  style={{ width: `${item.percentage}%` }}
                />
              </div>
              <span className="text-xs font-body text-sugan-ink/40 w-8">
      ...
      <div className="border-t border-sugan-ink/10 pt-4">
        <p className="text-sm font-body text-sugan-ink/60 mb-3">Based on customer feedback:</p>
        <div className="grid grid-cols-2 gap-3">
          {aspectRatings.map((item) => (
            <div key={item.aspect} className="flex items-center justify-between">
              <span className="text-sm font-body text-sugan-ink">{item.aspect}</span>
              <div className="flex items-center gap-1">
                <Star className="w-3 h-3 text-sugan-gold fill-sugan-gold" />
                <span className="text-sm font-body text-sugan-ink font-medium">
