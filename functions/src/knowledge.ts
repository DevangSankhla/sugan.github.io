...
import { PRODUCTS, ROOMS_OVERVIEW, PRODUCT_COUNT, type CatalogProduct } from './knowledge.generated';
...
export function selectRelevantProducts(query: string): CatalogProduct[] {
  ...
  const scored = PRODUCTS.map((p) => {
    let score = 0;
    if (skus.has(p.id)) score += 100;
    for (const t of terms) if (p.search.includes(t)) score += 1;
    if (priceMax !== null && p.price <= priceMax) score += 0.5;
    return { p, score };
  });
...
export function buildSystemPrompt(latestUserMessage: string): string {
  ...
  const catalog =
    `RELEVANT PRODUCTS — a subset of our ${PRODUCT_COUNT}-product catalogue, matched to this question (not the full range). ` +
