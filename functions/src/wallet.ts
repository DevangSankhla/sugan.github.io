  ...
  await db().runTransaction(async (tx) => {
    ...
    if (next < 0) {
      throw new functions.https.HttpsError(
        'failed-precondition',
        `Insufficient balance: current ₹${current}, cannot subtract ₹${Math.abs(amount)}`,
      );
    }
    balanceAfter = next;
...
// ---------------------------------------------------------------------------
export const redeemWalletForOrder = functions.https.onCall(async (data, context) => {
  if (!context.auth) {
    throw new functions.https.HttpsError('unauthenticated', 'Sign in required');
  }
...
// ---------------------------------------------------------------------------
export const refundWalletOnOrderFailure = functions.firestore
  ...
  .onUpdate(async (change, context) => {
    ...
    if (walletApplied <= 0 || after.walletRefunded === true) return;
    if (after.paymentStatus === 'paid') return; // never refund a paid order

    const failedNow =
      (before.paymentStatus !== 'failed' && after.paymentStatus === 'failed') ||
